...
Si bien mantener una eficacia alta durante los partidos ayuda a controlar mejor el juego y tener mayor margen de maniobra, anotar los penales en momentos claves impacta más el resultado final. Se muestran 4 partidos donde anotar estos lanzamientos ayudó a ganar puntos del partido. 
...
En este partido, Bielorusia tuvo una eficacia del 100% en tiros de 7 metros. Se destaca que anotó sus 3 tiros en los últimos 10 minutos, cada uno para poder igualar el marcador. El partido terminó 32-32, por lo que ser eficaz en el partido en general y en momentos clave en particular influyó positivamente en obtener un empate con un rival a priori favorito. 
...
Gracias a su eficacia, Brasil logró empatar el partido contra uno de los equipos favoritos del torneo. A falta de 10 minutos, Brasil perdía por 3 goles. En el minuto 56:42, Brasil anota un penal cerca del final de encuentro, que lo mantiene con paridad en el marcador en ese momento. El partido finalizó en un empate, por lo que Brasil logró un punto, en parte gracias a su capacidad en los penales. 
...
En un partido donde ambos equipos tuvieron la delantera por distintos periodos del partido, Marruecos necesitó un tiro de 7 metros a falta de 7 segundos para lograr alargar el encuentro para entrar a la prórroga. Finalmente Marruecos obtuvo la victoria, por lo que anotar el tiro al final del tiempo reglamentario derivó finalmente en mejorar la clasificación final. 
...
Dinamarca logró la clasificación a semifinales tras tener una eficacia mayor que Egipto en la tanda final de 7 metros. Dinamarca terminó siendo campeona del mundial, en parte gracias a su capacidad de anotación en los penales.
...
## Casos con potencial pérdida de puntos por fallos 
...
Islandia aparece con 3 derrotas por 2 goles en los partidos de la main round. Es relevante mencionar que en los 3 partidos, Islandia falló 2 tiros de 7 metros; con esto, terminó con una eficacia en cada partido menor a su propio promedio en el torneo. 
...
Islandia falló un 7 metros a falta de menos de 10 minutos del final, con una diferencia de un gol del rival tanto en el partido contra Suiza como contra Francia (observaciones 10 y 11 de la tabla). 
...
Se genera la columna _peso_ponderado_lanz_7m_falladas_, la cual describe el peso que en conjunto tuvieron los lanzamientos de 7m fallados en cada partido. Dos de los partidos de Islandia aparecen en el top-6 según esta métrica. Además, en este top-6 es el único equipo con más de un lanzamiento fallado en el encuentro De esto se puede concluir que un mejor tirador de 7 metros pudo haber hecho la diferencia en el partido. 
...
De este análisis, se excluyen aquellos jugadores que solamente hicieron un penal en el torneo.
Además, se excluye Cabo Verde, equipo que solamente participó en un partido.



Se obtiene una correlación positiva (0.53) considerablemente alta entre el porcentaje de especialización de lanzadores de 7m y la eficacia del equipo. Es decir, cuanto mayor la especialización, más eficaz el equipo (esto no implica necesariamente causalidad). Además, se observa una correlación negativa (-0.44) entre la cantidad de tiradores en un equipo y su eficacia. Es decir, cuantos menos jugadores lanzan 7m, más eficaz es el equipo en general. 
...
Además, con un modelo lineal simple es posible verificar la validez estadística de la especialización para describir la eficacia.

**Nota: La muestra de tomar solamente mundial puede no ser significativa par el estudio, por lo que se tienen que tomar estos resultados con cuidado**

Si bien se podría hacer un estudio completo para verificar la validez del modelo, con la visión del resumen básico se observa que los residuos parecen seguir el supuesto de normalidad (centrados en cero, rangos intercuantiles simétricos y los gráficos). Además, se observa que el valor-p de nuestra variable _porcentaje_tiros_especialista_ es de 0.002 por lo que esta es estadísticamente significativa. Finalmente, se observa que el $R^2$ es de 0.28, por lo que es posible decir que con este modelo de tan solo una variable, podemos explicar un 28% de la varianza en la eficacia. 
...
Al estudiar cómo distribuyen los equipos sus tiros de penal, se observó que Islandia es el undécimo equipo en especialización de su tirador de 7 metros. Si bien está por encima del promedio en esta métrica, hay margen de mejora. 
...
ui <- dashboardPage(
  ...
  dashboardSidebar(collapsed = FALSE,
                   sidebarMenu(
                     menuItem("Marcador de Tiros de 7m", tabName = "dashboard", icon = icon("futbol")),
                     menuItem('Escogencia de Jugadores', tabName = 'escogencia_M', icon = icon("people-carry"))
                   )
  ),
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = 'escogencia_M',
              column(width = 3,
                     checkboxGroupButtons(inputId = "escogencia_mayor", "Escogencia Jugadores Entrenamiento Mayor",
                                          c(paste0('jugador_', 1:15), 
                                            c('Invitado_1', 'Invitado_2', 'Invitado_3')), 
                                          selected = paste0('jugador_', 1:15),
                                          status = "primary", 
                                          direction = 'vertical',
                                          checkIcon = list(
                                            yes = icon("ok", 
...
server <- function(input, output, session) {
  ...
  jugadores_mayor <- paste0('jugador_', 1:15)
  
  
  jugadores <- unique(c(jugadores_mayor, 'Invitado_1', 'Invitado_2', 'Invitado_3'))
  
  
  porteros_mayor <- paste0('portero_', 1:4)
  porteros <- unique(c(porteros_mayor, 'Invitado1'))
